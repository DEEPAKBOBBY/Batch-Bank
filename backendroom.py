...
def roomch():
    ...
    if request.method == 'POST':
        try:
            ...
            session['room'] = db
            mydb4 = mysql.connector.connect(host="localhost", user="root", passwd="deepakbobby26229", database=db)
            myc4 = mydb4.cursor()
            if mydb4:
...
def creatingdb():
    ...
    if request.method=='POST':
        ...
        mydb4 = mysql.connector.connect(host="localhost", user="root", passwd="deepakbobby26229", database="testt")
        myc4 = mydb4.cursor()
        try:
            ...
            if res4:
                ...
                try:
                    minedb = mysql.connector.connect(host="localhost", user="root", passwd="deepakbobby26229",
                                                     database=dbb)
                    minec = minedb.cursor()
                    ss2 = "CREATE TABLE sign (username VARCHAR(45) , pass VARCHAR(45))"
...
def signup():
    if request.method=='POST':
        ...
        mydb = mysql.connector.connect(host="localhost", user="root", passwd="deepakbobby26229", database=db)
        myc = mydb.cursor()
        try:
            qq4 = "insert into sign (username,pass) values(%s,%s)"
            values = (s1, s2)
...
def enterbill():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                mydbb = mysql.connector.connect(host="localhost", user="root", passwd="deepakbobby26229", database=db)
                mycc = mydbb.cursor()
...
def summaryy():
    ...
    try:
        mydb = mysql.connector.connect(host="localhost", user="root", passwd="deepakbobby26229", database=room_name)
        myc = mydb.cursor()
